...
- X Viral Monitor — Loon HTML 注入脚本
- 拦截 x.com 的 HTML 响应，把 content script 塞进 <head>
  */
...
const script = `<script> (function () { // ─── 样式 ────────────────────────────────────────────────────────────────── function injectStyle() { if (document.getElementById('xvm-style')) return; const s = document.createElement('style'); s.id = 'xvm-style'; s.textContent = \`
...
if (GRAPHQL_RE.test(url)) res.clone().json().then(scan).catch(()=>{});
} catch(e) {}
return res;
};
